...
# Verifica se todos os arquivos .b64 estão presentes
...
# Decodifica cada parte e escreve em um arquivo .bin temporário
$tempFiles = @()
for ($i=1; $i -le $totalParts; $i++) {
    $fname = "parte_" + $i.ToString("000") + ".b64"
    $outFile = "temp_$i.bin"
...
# Combina todos os arquivos .bin em um único arquivo (todos.bin) usando FileStream
...
# Processa o todos.bin linha por linha (sem carregar tudo na memória)
...
Write-Host "============================================================" -ForegroundColor Green
Write-Host "Todos os arquivos foram reconstruídos."
